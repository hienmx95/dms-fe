...
export class KpiItemsReportRepository extends Repository {
  constructor() {
    super(httpConfig);
    this.setBaseURL(url(API_BASE_URL, API_KPI_ITEMS_REPORT_ROUTE));
  }
  public count = (filter?: KpiItemsReportFilter): Promise<number> => {
    return this.http
      .post<number>(kebabCase(nameof(this.count)), filter)
      .then((response: AxiosResponse<number>) => response.data);
  };

  public list = (filter?: KpiItemsReportFilter): Promise<KpiItemsReport[]> => {
    return this.http
      .post<KpiItemsReport[]>(kebabCase(nameof(this.list)), filter)
      .then((response: AxiosResponse<KpiItemsReport[]>) => {
        return response.data?.map((model: PureModelData<KpiItemsReport>) =>
          KpiItemsReport.clone<KpiItemsReport>(model),
        );
      });
  };
  public get = (id: number | string): Promise<KpiItemsReport> => {
    return this.http
      .post<KpiItemsReport>(kebabCase(nameof(this.get)), { id })
      .then((response: AxiosResponse<KpiItemsReport>) =>
        KpiItemsReport.clone<KpiItemsReport>(response.data),
      );
  };
...
export const kpiItemsReportRepository: KpiItemsReportRepository = new KpiItemsReportRepository();
