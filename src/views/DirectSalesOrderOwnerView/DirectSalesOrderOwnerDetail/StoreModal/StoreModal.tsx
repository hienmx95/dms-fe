...
import Table, {
  ...
  RowSelectionType, TableRowSelection,
} from 'antd/lib/table';
...
function StoreModal<T extends Model>(props: ContentModalProps<T>) {
  ...
  const [
    ...
    total,
    setLoadList,
  ] = directSalesOrderOwnerService.useStoreContentMaster(
    getList,
    count,
    saleEmployeeId,
  );
  ...
  const columns: ColumnProps<Store>[] = React.useMemo(() => {
    return [
      ...
      {
        title: translate('indirectSalesOrders.stores.codeDraft'),
          ...
          <Form>
            <Row>
              ...
              <Col className="pl-1" span={6}>
                <FormItem
                  className="mb-0"
                  label={translate('indirectSalesOrders.stores.codeDraft')}
                  labelAlign="left"
                >
                  <AdvancedStringFilter
                    filterType={nameof(filterStore.codeDraft.contain)}
                    filter={filterStore.codeDraft}
                    onChange={handleChangeFilter(nameof(filterStore.codeDraft))}
                    placeholder={translate(
                      'indirectSalesOrders.stores.placeholder.code',
                    )}
                    className="w-100"
              ...
              <Col className="pl-1" span={6}>
                ...
                >
                  <AdvancedIdFilter
                    ...
                    onChange={handleChangeFilter(nameof(filterStore.storeGroupingId))}
                    getList={directSalesOrderOwnerRepository.singleListStoreGrouping}
            ...
            <Row>
              <Col className="pl-1" span={6}>
                ...
                >
                  <AdvancedIdFilter
                    ...
                    onChange={handleChangeFilter(nameof(filterStore.storeTypeId))}
                    getList={directSalesOrderOwnerRepository.singleListStoreType}
              ...
              <Col className="pl-1" span={6}>
                ...
                >
                  <AdvancedTreeFilter
                    ...
                    onChange={handleChangeFilter(nameof(filterStore.organizationId))}
                    modelFilter={organizationFilter}
                    setModelFilter={setOrganizationFilter}
                    getList={directSalesOrderOwnerRepository.filterListOrganization}
              ...
              <Col className="pl-1" span={6}>
                ...
                >
                  <AdvancedIdFilter
                    filter={filterStore.storeStatusId}
                    filterType={nameof(filterStore.storeStatusId.equal)}
                    value={filterStore.statusId.equal}
                    onChange={handleChangeFilter(nameof(filterStore.storeStatusId))}
                    getList={
