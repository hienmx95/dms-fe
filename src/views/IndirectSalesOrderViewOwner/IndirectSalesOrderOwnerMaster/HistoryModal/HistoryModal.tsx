...
function HistoryModal<T extends Model>(props: ContentModalProps<T>) {
  ...
  const { toggle, isOpen, title, handleClose, list } = props;
  const columns: ColumnProps<InventoryHistory>[] = React.useMemo(() => {
    return [
      {
        title: translate('indirectSalesOrders.history.role'),
      ...
      {
        title: translate('indirectSalesOrders.history.appUser'),
          ...
          return (
            <>
              {content?.workflowState &&
                content.workflowState.code === 'APPROVED' && (
                  <>
                    {appUser.username} - {appUser.displayName}
                  </>
                )}
              {content?.workflowStateId === 4 && (
                <>
                  {appUser.username} - {appUser.displayName}
                </>
              )}
              {content?.workflowState &&
                content.workflowState.code === 'PENDING' && (
                  <>
                    {content.nextApprovers &&
                      content.nextApprovers.length > 0 &&
                      content.nextApprovers.map(
                        (nextApprover: AppUser, index: number) => {
                          if (index === content.nextApprovers.length - 1) {
                            return (
                              <>
                                {nextApprover.username} -
                                {nextApprover.displayName}
                              </>
                            );
                          }
                          return (
      ...
      {
        title: translate('indirectSalesOrders.history.workflowState'),
      ...
      {
        title: translate('indirectSalesOrders.history.updateTime'),
