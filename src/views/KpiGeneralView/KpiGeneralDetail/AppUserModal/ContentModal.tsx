...
function ContentModal<T extends Model>(props: ContentModalProps<T>) {
  ...
  const [
    ...
    total,
    setLoadListAppUser,
  ] = kpiGenralService.useAppUserContentMaster(getList, count, currentItem, currentKpiYear);
  ...
  React.useEffect(() => {
    ...
    setLoading(false);
    if (isOpen === true) {
      setLoadListAppUser(true);
    }
    handleSearch();
  }, [setListAppUser, setTotal, setLoading, list, totalAppUser, setLoadListAppUser, handleSearch, isOpen]);
