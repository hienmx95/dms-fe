...
import { ShowingOrderWithdrawContentFilter } from 'models/posm/ShowingOrderWithdrawContentFilter';
...
export interface ShowingOrderContentTableProps
  extends ContentTableProps<ShowingOrderWithdraw, ShowingOrderWithdrawContent> {
...
function ShowingOrderContentTable(props: ShowingOrderContentTableProps) {
  ...
  >(model, setModel, nameof(model.showingOrderContentWithDraws));

  const [
    showingOrderContentFilter,
    setShowingOrderContentFilter,
  ] = React.useState<ShowingOrderWithdrawContentFilter>(
    new ShowingOrderWithdrawContentFilter(),
  );
  const [
    dataSource,
    pagination,
  ...
  const rowSelection: TableRowSelection<ShowingOrderWithdrawContent> = crudService.useContentModalList<
    ShowingOrderWithdrawContent
    ...
    listItem => {
      if (listItem && listItem.length > 0) {
        ...
        const contents = listItem.map((item: ShowingItem) => {
          const content = new ShowingOrderWithdrawContent();
    ...
    (index: number) => {
      return () => {
        ...
        const selectedShowingItem = showingOrderContents.map(
          (content: ShowingOrderWithdrawContent) => {
      ...
      onOk() {
        if (selectedContents && selectedContents.length > 0) {
          const selectedStoreIds = selectedContents.map(
            (content: ShowingOrderWithdrawContent) => content.showingItemId,
          );
  ...
  const columns: ColumnProps<ShowingOrderWithdrawContent>[] = React.useMemo(
    () => [
      {
        title: translate(generalLanguageKeys.columns.index),
        key: nameof(generalLanguageKeys.columns.index),
        width: generalColumnWidths.index,
        render: renderMasterIndex<ShowingOrderWithdrawContent>(pagination),
      },
      {
      ...
      },
      {
        ...
        render(
          quantity: any,
          showingOrderContent: ShowingOrderWithdrawContent,
          index,
        ) {
          return (
            <>
      ...
      {showingOrderContents?.length === 0 && (
        <FormItem
          validateStatus={formService.getValidationStatus<ShowingOrderWithdraw>(
            model.errors,
            nameof(model.showingOrderContentWithDraws),
          )}
          help={model.errors?.showingOrderContentWithDraws}
