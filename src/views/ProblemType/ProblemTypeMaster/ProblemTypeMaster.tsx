...
import './ProblemTypeMaster.scss';
import ProblemTypePreview from './ProblemTypePreview';

const { Item: FormItem } = Form;

function ProblemTypeMaster() {
  ...
  const [
    ...
    resetSelect,
    setResetSelect,
  ] = crudService.useMaster<ProblemType, ProblemTypeFilter>(
    ProblemType,
    ProblemTypeFilter,
    problemTypeRepository.count,
    problemTypeRepository.list,
   ...
   */
  const [rowSelection, hasSelected] = tableService.useRowSelection<ProblemType>(
    [],
    undefined,
    resetSelect,
    setResetSelect,
  ...
  const [handleDelete] = tableService.useDeleteHandler<ProblemType>(
    problemTypeRepository.delete,
    setLoading,
    list,
    setList,
    handleDefaultSearch,
  ...
  const [handleBulkDelete] = tableService.useBulkDeleteHandler(
    ...
    problemTypeRepository.bulkDelete,
    setLoading,
    handleDefaultSearch,
  );
  ...
  const columns: ColumnProps<ProblemType>[] = React.useMemo(
    () => {
      return [
        {
          ...
          render: renderMasterIndex<ProblemType>(pagination),
        },
        {
          title: translate('problemTypes.code'),
          key: nameof(list[0].code),
          dataIndex: nameof(list[0].code),
          sorter: true,
          align: 'center',
          sortOrder: getOrderTypeForTable<ProblemType>(
            nameof(list[0].code),
            sorter,
          ),
        },
        ...
        {
          title: translate('problemTypes.name'),
        ...
        {
          title: translate('problemTypes.status'),
...

        {
          ...
          align: 'center',
          render(id: number, problemType: ProblemType) {
            return (
              <div className="d-flex justify-content-center button-action-table">
                ...
                {validAction('update') && (
                  <Tooltip title={translate(generalLanguageKeys.actions.edit)}>
                    <button
                      className="btn btn-sm btn-link"
                      onClick={() => handleGoDetail(problemType)}
                    >
                ...
                {!problemType.used && validAction('delete') && (
                  <Tooltip
  ...
  return (
    <div className="page master-page">
      <Card title={translate('problemTypes.master.title')}>
        <CollapsibleCard
            ...
            <Row>
              ...
              <Col className="pl-1" span={6}>
                <FormItem
                  labelAlign="left"
                  className="mb-1"
                  label={translate('problemTypes.code')}
                >
              ...
              <Col className="pl-1" span={6}>
                <FormItem
                  labelAlign="left"
                  className="mb-1"
                  label={translate('problemTypes.name')}
                >
                  <AdvancedStringFilter
              ...
              {validAction('filterListStatus') && (
                <Col className="pl-1" span={6}>
                  <Form.Item
                    labelAlign="left"
                    className="mb-1"
                    label={translate('problemTypes.status')}
                  >
                    <AdvancedIdFilter
                      ...
                      getList={problemTypeRepository.filterListStatus}
      ...
      {visible === true && (
        <ProblemTypeDetail
          ...
          getListProblemType={problemTypeRepository.list}
          setListProblemType={setList}
...
export default ProblemTypeMaster;
